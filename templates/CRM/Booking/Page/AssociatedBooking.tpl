<table class="selector">
  ...
  <tr id="rowid{$row.id}" class="{cycle values="odd-row,even-row"} crm-booking_{$row.id}">
    <td class="crm-booking-contact">
      <a href="{crmURL p="civicrm/contact/view" q="reset=1&cid=`$row.priamry_contact_id`"}" id="view_contact" title="{ts}View Booking Contactr Record{/ts}">{$row.primary_contact_name}</a>
    </td>
