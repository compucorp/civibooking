...
{if $action eq 1 or $action eq 2 or $action eq 8}
   {include file="CRM/Admin/Form/AdhocChargesItem.tpl"}
{else}
{if $rows}
<div id="ltype">
        {strip}
        ...
        <div class="action-link">
          <a href="{crmURL q="action=add&reset=1"}" id="new" class="button"><span><div class="icon add-icon"></div>{ts}Add Additional Charges Item{/ts}</span></a>
        </div>
        {/if}
</div>
{elseif $action ne 1}
