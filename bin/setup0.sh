...
CIVIROOT="/Users/timothylim/repos/htdocs/civicrm"
...
function buildDAO() {
  ...
  [ ! -d "$EXTROOT/CRM/Booking/DAO/" ] && mkdir -p "$EXTROOT/CRM/Booking/DAO/"
  cp -f "$CIVIROOT/CRM/Booking/DAO"/* "$EXTROOT/CRM/Booking/DAO/"
}

##############################
function cleanup() {
  for DIR in "$XMLBUILD" "$CIVIROOT/CRM/Booking" "$EXTROOT/CRM/Booking/DAO/" ; do
    if [ -e "$DIR" ]; then
      rm -rf "$DIR"
    fi
  done
